...
fn run(args: &[&str], expected_file: &str) -> Result<()> {
    let mut file = std::fs::File::open(expected_file)?;
...
fn skip_bad_file() -> Result<()> {
    ...
    cargo_bin_cmd!().args([EMPTY, &bad, ONE])
	.assert()
...
fn die_bad_bytes() -> Result<()> {
    let expected = format!(
	"invalid value 'foo' for '--bytes <BYTES>': invalid digit found in string"
    );

    cargo_bin_cmd!().args(["-c", "foo", EMPTY])
	.assert()
	.failure()
...
#[test]
fn die_bad_lines() -> Result<()> {
    let expected = format!(
	"invalid value 'foo' for '--lines <LINES>': invalid digit found in string"
    );
    
    cargo_bin_cmd!().args(["-n", "foo", EMPTY])
        .assert()
        .failure()
        .stderr(predicate::str::contains(expected));
